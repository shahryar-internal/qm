...
it anywhere. Agent View messages use Slack Agent Sessions: status moves through processing,
active, suspended, or closed; responses stream with plan and task progress; and Slack's native
stop button cancels the exact durable run. Suggested prompts come from the manifest. Active-view
context is stored as bounded Slack identifiers under the exact workspace, owner, DM, and thread,
then passed through the normal gateway context for that owner's turn only.
...
- **Existing installs must update the manifest and reinstall** to pick up the
  `assistant:write` scope and the `app_home_opened`, `app_context_changed`,
  `agent_session_stopped`, and `agent_session_title_changed` events. The older
  `assistant_thread_started` and `assistant_thread_context_changed` subscriptions remain as a
  bounded compatibility bridge. Installs that don't update keep ordinary messaging but lack the
  complete Agent Sessions experience.
...
| Agent View `app_context` on `message.im`                                                                         | bounded active-view identifiers persisted by workspace + Slack user + DM + thread, then supplied only to that exact owner's gateway context                                                                                                                                                                                                                                                                                                                                                                                                                            |
| `agent_session_stopped`                                                                                          | selects the durable active run only when workspace, user, DM/channel, thread, event time, and streaming-message checkpoint match; stores a durable abort signal and restores the session to `active`                                                                                                                                                                                                                                                                                                                                                                   |
...
| Agent View DM thread                                                                                             | `threadRef` = `dm:<channel>:<root>` (one durable core session per Slack Agent Session thread); ordinary non-Agent DM fallback remains channel-scoped                                                                                                                                                                                                                                                                                                                                                                                                                   |
