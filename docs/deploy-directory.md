...
| `approvals`                 | Appended to the command-policy floor. A rule may deny or require approval for its own tool. An allow must declare `subsumesToolApproval: true` and use an anchored, single-command pattern; it cannot override administrator policy.                                   |
