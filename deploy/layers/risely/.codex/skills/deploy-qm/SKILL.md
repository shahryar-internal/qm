...
This existing Risely deployment intentionally uses a deferred custom Gemini
provider rather than one of the initializer's built-in providers. Preserve the
missing `modelProvider`, start the core and authenticated Admin surfaces, then
follow `GEMINI-SETUP.md` before `check --live`. Do not use an unauthenticated
