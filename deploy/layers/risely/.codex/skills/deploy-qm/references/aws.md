...
Use the deployment-owned `aws:image:build` wrapper for this Risely layer. It
applies the reviewed aarch64 GitHub CLI fix to the pinned QM builder and restores
`node_modules` on success or failure. Do not bypass it with the underlying
`npm exec qm -- infra build-image` command.

This Risely deployment starts without `modelProvider` because Gemini is a
custom OpenAI-compatible provider. After `up`, sign in to Admin and complete
`GEMINI-SETUP.md`. Only after Gemini is validated and selected as the base
model, run:
...
teardown section. The RDS instance has deletion protection enabled by default;
disabling it requires a separate, narrowly reviewed Terraform apply against the
exact pilot database before destroy can proceed.
