locals {
  tags                       = { Deployment = var.org_id, Environment = "pilot", ManagedBy = "terraform" }
  ...
  task_role_arns = distinct(compact(concat(
    ...
    [for name, service in var.services : service.task_role_arn if name != local.ceo_canary_service_name],
  )))
  execution_role_arns = distinct(compact(concat(
    [local.default_execution_role_arn],
    [for name, service in var.services : service.execution_role_arn if name != local.ceo_canary_service_name],
  )))
}
...
resource "aws_iam_role_policy" "task_secrets" {
  ...
  policy = jsonencode({ Version = "2012-10-17", Statement = [{ Effect = "Allow", Action = ["secretsmanager:GetSecretValue"], Resource = [for name, secret in aws_secretsmanager_secret.contract : secret.arn if !startswith(name, "CANARY_")] }] })
}
...
resource "aws_iam_user" "auth_smtp" {
  name = "${var.cluster_name}-auth-smtp"
  tags = local.tags
}
...
resource "aws_iam_user_policy" "auth_smtp" {
  ...
  policy = jsonencode({
    ...
    Statement = [{
      ...
      Resource = "*"
      Condition = {
        StringLike = { "ses:FromAddress" = "*@notifications.risely.ai" }
      }
    }]
  })
}
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        ...
        Resource = [for name, service in var.services : "arn:aws:ecs:${var.region}:${data.aws_caller_identity.current.account_id}:task-definition/${service.ecs_service}:*" if name != local.ceo_canary_service_name]
      },
      {
        ...
        Resource = [for name, repository in aws_ecr_repository.service : repository.arn if name != local.ceo_canary_service_name]
      },
      {
        Sid      = "DescribeCluster"
        Effect   = "Allow"
      ...
      },
      {
        ...
        Resource = [for name, service in var.services : "arn:aws:ecs:${var.region}:${data.aws_caller_identity.current.account_id}:service/${var.cluster_name}/${service.ecs_service}" if name != local.ceo_canary_service_name]
      },
      {
      ...
      },
      {
        ...
        Resource = [for name, secret in aws_secretsmanager_secret.contract : secret.arn if !startswith(name, "CANARY_")]
      },
      {
        Sid      = "TailServiceLogs"
        Effect   = "Allow"
        Action   = ["logs:FilterLogEvents", "logs:GetLogEvents", "logs:DescribeLogStreams", "logs:StartLiveTail"]
        Resource = concat([for name, group in aws_cloudwatch_log_group.service : group.arn if name != local.ceo_canary_service_name], [for name, group in aws_cloudwatch_log_group.service : "${group.arn}:*" if name != local.ceo_canary_service_name])
      },
      {
        Sid      = "DeployLease"
        Effect   = "Allow"
...
resource "aws_db_instance" "this" {
  ...
  vpc_security_group_ids    = [aws_security_group.database.id, aws_security_group.ceo_canary_database.id]
...
resource "aws_secretsmanager_secret_version" "database" {
  ...
  secret_string = "postgresql://${var.db_username}:${urlencode(random_password.database.result)}@${aws_db_instance.this.address}:5432/${var.db_name}"
}
...
resource "aws_ecs_task_definition" "bootstrap" {
  for_each                 = { for name, service in var.services : name => service if name != local.ceo_canary_service_name }
