...
variable "ceo_canary_runtime_image" {
  type = string
  validation {
    condition     = can(regex("^[0-9]{12}\\.dkr\\.ecr\\.[a-z0-9-]+\\.amazonaws\\.com/[a-z0-9][a-z0-9._/-]*@sha256:[0-9a-f]{64}$", var.ceo_canary_runtime_image))
    error_message = "ceo_canary_runtime_image must be a complete ECR repository URI pinned by sha256 digest"
  }
}
