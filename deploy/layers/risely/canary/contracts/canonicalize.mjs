...
function canonical(value, stack, depth, budget) {
  ...
  try {
    ...
    if (Object.values(descriptors).some((descriptor) => !descriptor.enumerable)) {
      throw new TypeError("Canonical JSON does not support non-enumerable properties");
    }
    if (Object.values(descriptors).some((descriptor) => descriptor.get || descriptor.set)) {
      throw new TypeError("Canonical JSON does not support accessor properties");
    }
...
export function deepFreeze(value, seen = new Set()) {
  ...
  if (types.isProxy(value)) throw new TypeError("Cannot freeze a proxy");
  seen.add(value);
  const descriptors = Object.getOwnPropertyDescriptors(value);
  if (Object.values(descriptors).some((descriptor) => descriptor.get || descriptor.set)) {
    throw new TypeError("Cannot freeze accessor properties");
  }
  for (const descriptor of Object.values(descriptors)) deepFreeze(descriptor.value, seen);
