...
session canary. This Risely deployment must first complete the authenticated
custom-provider registration in `GEMINI-SETUP.md`. The task is complete only after that gate passes, the
...
This Risely deployment intentionally takes that deferred route for its custom
OpenAI-compatible Gemini provider. Follow `GEMINI-SETUP.md` after the core and
Admin surfaces start. Register the provider through the authenticated Admin
surface, validate it, and select `gemini-3.7-flash` before any live canary.

After a fresh database restore or bootstrap, apply and verify the pilot's
fail-closed shell policy:

```bash
npm run governance:lock
```

This sets the Risely organization command policy to an empty allowlist. Shell
commands remain denied even when a one-session QM tool approval is granted.
Add narrowly reviewed allowlist entries only when a workflow requires them.
...
Follow the selected provider reference. For this Risely deployment, complete
the authenticated provider and base-model steps in `GEMINI-SETUP.md`, then run:
